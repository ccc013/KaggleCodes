...
# 用户在截止日前 最后一个活跃日的交互商品数 和 活跃日日均交互商品数 的差值 除以 活跃日日均商品交互数（去中心化）
...
def feat_set(all_action,train_start_date, train_end_date, label_start_date= '2016-02-28', 
    ...
    if isTrain:
        ...
        del all_feat['user_id']
        del all_feat['sku_id']
        return user_sku,all_feat, feat_list
...
if __name__ == '__main__':
    ...
    print '{} generating test data:'.format(datetime.now())
    test_user_sku, test_data,feat_list= make_test_set(all_action,test_start_date, test_end_date)
    ...
    # 执行特征筛选
    test_data_select=delfeat(test_data,delfeatlist)
    print 'test_data_select shape:', test_data_select.shape
    print "保存 test_user_sku525.csv"
    test_user_sku.to_csv('tmp/test_user_sku525.csv', index=False, index_label=False)
    print "保存 test_data525.csv"
    test_data_select.to_csv('tmp/test_data525.csv', index=False, index_label=False)
